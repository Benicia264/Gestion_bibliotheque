...
// Si on agrandit l'écran alors que le menu
// mobile était ouvert
...
const themeSauvegarde =
  localStorage.getItem("bibliotheque-theme");


// Si un thème existe déjà,
// on l'applique
...
function mettreAJourTheme() {
  ...
  if (modeNuit) {
    ...
    themeLabel.textContent = "Mode jour";

  } else {

    themeIcon.textContent = "☾";
    themeLabel.textContent = "Mode nuit";
...
async function chargerStatistiques() {
  try {
    ...
    document.getElementById("stat-livres").textContent =
      s.totalLivres ?? "—";

    document.getElementById("stat-adherents").textContent =
      s.totalAdherents ?? "—";

    document.getElementById("stat-en-cours").textContent =
      s.empruntsEnCours ?? "—";

    document.getElementById("stat-retard").textContent =
      s.empruntsEnRetard ?? "—";

    document.getElementById("stat-top-livre").textContent =
      s.livrePlusEmprunte?.titre ??
      "Aucun emprunt pour l'instant";

    document.getElementById("stat-top-adherent").textContent =
      s.adherentPlusActif?.nom ??
      "Aucun emprunt pour l'instant";
...
async function chargerAuteurs() {
  ...
  tbody.innerHTML =
    `<tr><td colspan="3" class="empty-row">Chargement…</td></tr>`;
  ...
  try {
    ...
    if (!auteursCache.length) {
      tbody.innerHTML =
        `<tr><td colspan="3" class="empty-row">Aucun auteur enregistré.</td></tr>`;
      return;
    }
      ...
      <tr>
        ...
        <td class="row-actions">
          <button class="btn btn-ghost btn-sm"
            onclick="ouvrirModifAuteur(${a.id})">
            Modifier
          </button>

          <button class="btn btn-danger btn-sm"
            onclick="supprimerAuteur(${a.id})">
            Supprimer
          </button>
        </td>
      </tr>
    ...
    tbody.innerHTML =
      `<tr><td colspan="3" class="empty-row">Erreur de chargement.</td></tr>`;
  }
}

async function supprimerAuteur(id) {
  if (!confirm("Supprimer cet auteur ? Les livres associés pourraient être affectés.")) {
    return;
  }

  try {
    await appel(`/auteurs/${id}`, {
      method: "DELETE"
    });
...
function ouvrirNouvelAuteur() {
  ...
  `, async () => {
    ...
    const nationalite =
      document.getElementById("f-nationalite").value.trim();

    if (!nom || !nationalite) {
      afficherErreurForm(
        "Nom et nationalité sont obligatoires."
      );
      return;
    }
...
document
  .getElementById("btn-nouvel-auteur")
  .addEventListener("click", ouvrirNouvelAuteur);
...
async function chargerLivres() {
  ...
  tbody.innerHTML =
    `<tr><td colspan="5" class="empty-row">Chargement…</td></tr>`;

  try {
    const data = await appel(
      `/livres?recherche=${encodeURIComponent(livresRecherche)}&page=${livresPage}&limite=8`
    );

    if (!data.livres.length) {

      tbody.innerHTML =
        `<tr><td colspan="5" class="empty-row">Aucun livre trouvé.</td></tr>`;
    ...
    tbody.innerHTML =
      `<tr><td colspan="5" class="empty-row">Erreur de chargement.</td></tr>`;
  }
}

function dessinerPagination(page, totalPages) {
  const el =
    document.getElementById("livres-pagination");
...
document
  .getElementById("livres-recherche")
  .addEventListener("input", (e) => {
    ...
    window._rechercheTimeout =
      setTimeout(chargerLivres, 300);
  });
...
document
  .getElementById("btn-nouveau-livre")
  .addEventListener("click", ouvrirNouveauLivre);
...
async function chargerAdherents() {
  ...
  const tbody =
    document.getElementById("adherents-tbody");
  ...
  try {
    ...
    const adherents =
      await appel("/adherents");

    if (!adherents.length) {

      tbody.innerHTML =
        `<tr><td colspan="3" class="empty-row">Aucun adhérent inscrit.</td></tr>`;
    ...
    tbody.innerHTML = adherents.map(a => `
      <tr>
        ...
        <td class="row-actions">
          ...
          <button
            class="btn btn-danger btn-sm"
            onclick="supprimerAdherent(${a.id})">
            Supprimer
          </button>
...
async function voirHistorique(id, nom) {
  ...
  try {
    ...
    const historique =
      await appel(`/adherents/${id}/emprunts`);
    ...
    const contenu = historique.length
      ...
      ? `<ul style="margin:0;padding-left:18px;">
          ${historique.map(h =>
            `<li>
              ${h.livre_titre}
              — emprunté le ${formaterDate(h.date_emprunt)}
              ${
                h.date_retour_effective
                  ? `, rendu le ${formaterDate(h.date_retour_effective)}`
                  : ' (en cours)'
              }
            </li>`
          ).join("")}
...
document
  .getElementById("btn-nouvel-adherent")
  .addEventListener("click", () => {
    ...
    ouvrirModale("Ajouter un adhérent", `
      ...
      <div class="field">
        ...
        <label for="f-date-inscription">
          Date d'inscription
        </label>

        <input
          id="f-date-inscription"
          type="date"
          required
          value="${aujourdHui}">
    ...
    `, async () => {
      ...
      const date_inscription =
        document.getElementById("f-date-inscription").value;

      if (!nom || !contact || !date_inscription) {
...
async function chargerEmprunts() {
  ...
  const tbody =
    document.getElementById("emprunts-tbody");

  tbody.innerHTML =
    `<tr><td colspan="6" class="empty-row">Chargement…</td></tr>`;

  try {

    // IMPORTANT :
    // On récupère uniquement les emprunts encore en cours.
    // Un emprunt avec une date_retour_effective
    // n'apparaîtra donc plus ici.

    const emprunts =
      await appel("/emprunts/en-cours");

    if (!emprunts.length) {

      tbody.innerHTML =
        `<tr><td colspan="6" class="empty-row">Aucun emprunt en cours.</td></tr>`;
    ...
    tbody.innerHTML = emprunts.map(e => {
      ...
      const enRetard =
        new Date(e.date_retour_prevue) < new Date();
      ...
      return `
        <tr>
          ...
          <td class="row-actions">
            ...
            <button
              class="btn btn-ghost btn-sm"
              onclick="enregistrerRetour(${e.id})">
              Marquer rendu
            </button>
    ...
    tbody.innerHTML =
      `<tr><td colspan="6" class="empty-row">Erreur de chargement.</td></tr>`;
  }
}
...
async function enregistrerRetour(id) {
  ...
  try {
    ...
    await appel(`/emprunts/${id}/retour`, {
      method: "PUT"
    });
    ...
    // 1. la liste des emprunts en cours
    // 2. le statut du livre
    // 3. les statistiques
...
document
  .getElementById("btn-nouvel-emprunt")
  .addEventListener("click", async () => {
    ...
    try {
      ...
      [adherents, livresDispo] =
        await Promise.all([

          appel("/adherents"),

          appel("/livres?limite=200")
            .then(d =>
              d.livres.filter(
      ...
      // On affiche quand même le formulaire
    ...
    `, async () => {
      ...
      const adherent_id =
        document.getElementById("f-adherent").value;

      const livre_id =
        document.getElementById("f-livre").value;

      const date_retour_prevue =
        document.getElementById("f-date-retour").value;
      ...
      try {
        ...
        // Actualiser les trois parties concernées
